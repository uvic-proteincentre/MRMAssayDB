...
function MutsNeedlePlot (config) {
    ...
    this.tip = d3.tip()
      ...
      .offset([-10, 0])
      .html(function(d) {
        return "<span>" +"Type:" + d.category +"<br/>Description:"+d.description +  "<br/> position: " + d.coordString + " & residue:"+ d.residuename+"</span>";
      });
    ...
    this.selectionTip = d3.tip()
        ...
        .offset([-50, 0])
        .html(function(d) {
            if (Math.round(d.left) >=0 && Math.round(d.right) <= maxCoord){
                return "<span> Selected residue range:<br/>" + Math.round(d.left) + " - " + Math.round(d.right) + "</span>";
            }
    ...
    selectionRect.on("mouseenter", function() {
        ...
        if (Math.round(selection[0]) >=0 && Math.round(selection[1]) <= maxCoord){
    ...
    self.on("needleSelectionChange", function(edata) {
            ...
            if (selection[1] - selection[0] > 0 && Math.round(selection[0]) >=0 && Math.round(selection[1]) <= maxCoord) {
                self.selectionTip.show({left: selection[0], right: selection[1]}, selectionRect.node());
                d3.select(".d3-tip-selection")
                    .transition()
                    .delay(3000)
...
MutsNeedlePlot.prototype.drawNeedles = function(svg, mutationData, regionData) {
    ...
    function formatMutationEntry(d) {
        ...
        residuename = d.residuename || "other";
